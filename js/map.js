...
function testsearchBuilding()
{
	while(mapUL.hasChildNodes())
	{
		...
		var srch = document.getElementById("testDropdownInput").value;
			...
			xhr.onreadystatechange = function()
			{
				if(this.readyState == 4 && this.status == 200)
				{
					...
					for(i=0; i < jsonObject['results'].length ; i++)
					{
						...
						a.appendChild(t);
						document.getElementById("testDropdownInput").appendChild(a);
						
						var span = document.createElement("SPAN");
						var txt = document.createTextNode("\u00D7");
						a.id = jsonObject['results'][i][0];
						a.setAttribute('onclick', "setBuildingTo(this.id)");
						span.appendChild(txt);
